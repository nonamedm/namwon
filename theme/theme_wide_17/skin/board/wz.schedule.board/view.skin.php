...
include_once(G5_PLUGIN_PATH.'/wz.schedule.board/lib/function.lib.php');
...
<article id="bo_v" style="width:<?php echo $width; ?>">
    ...
    <section id="bo_v_info">
        <h2>페이지 정보</h2>
        <div class="scdate-txt">
            <?php
            if ($view['wr_1'] == $view['wr_2']) {
                echo '<span class="date-txt">'.wz_get_hangul_date($view['wr_1']).'</span>';
            }
            else {
                echo '<span class="date-txt">'.wz_get_hangul_date($view['wr_1']).'</span> 부터 <span class="date-txt">'.wz_get_hangul_date($view['wr_2']).'</span> 까지';
            }
            ?>
        </div>
